...
$line = Get-Process |Sort-Object -Property CPU | Select-Object -Last 5
$compara = 1
write-host "{"
write-host " `"data`":[`n"
...
$percent = (get-wmiobject Win32_PerfFormattedData_PerfProc_Process | ? { $_.IDProcess -eq $2 } | Select).PercentProcessorTime
$cores = Get-WmiObject -Class Win32_Processor | Select-Object -Property NumberOfLogicalProcessors
$totalcores = ($cores | Measure-Object 'NumberOfLogicalProcessors' -Sum).Sum
$calc = $percent/$totalcores
[math]::Round($calc)
#Write-Output $calc
}
